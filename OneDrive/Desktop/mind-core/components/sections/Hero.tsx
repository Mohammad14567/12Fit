 ...
 * Hero — the eclipse.
 *
 * A planet-scale dark sphere sits just below the fold; only its upper rim is
 * visible, burning with a thin arc of light that fades toward the edges.
 * The type floats in the dark above the horizon. As you scroll, the text
 * lifts away and the arc dims — dawn receding as you enter the site.
 */
export default function Hero() {
  ...
  const headline: ReactNode[] = [
    "We build the",
    <>
      software the <span className="text-aurora">future</span>
    </>,
    "was waiting for.",
  ];
  ...
  return (
    ...
    >
      ...
      <motion.div
        ...
        className="relative z-10 mt-[26svh] flex flex-col items-center text-center md:mt-[28svh]"
      >
        <motion.p
          initial={{ opacity: 0, y: 12 }}
          animate={{ opacity: 1, y: 0 }}
          transition={{ duration: 0.8, ease: glide, delay: 0.15 }}
          className="inline-flex items-center gap-2 rounded-full border border-hairline bg-white/[0.04] px-3 py-1 backdrop-blur-md"
        >
          <span className="relative flex h-1 w-1">
            <span className="absolute inline-flex h-full w-full animate-ping rounded-full bg-beam opacity-75" />
            <span className="relative inline-flex h-1 w-1 rounded-full bg-beam shadow-[0_0_6px_rgba(156,194,255,0.9)]" />
          </span>
          <span className="label !text-frost/80">Mind Core · Software studio</span>
        </motion.p>

        <h1 className="display lit mt-5 text-3xl font-semibold leading-[1.04] sm:text-4xl md:text-[3.5rem]">
          {headline.map((line, i) => (
        ...
        <motion.p
          ...
          className="mt-4 max-w-md text-pretty text-sm leading-relaxed text-mist sm:text-base"
        >
          Digital products, AI systems, and automation — designed and engineered
          with the precision of the companies you wish had built your software.
        </motion.p>
        ...
        >
          ...
          <GlowButton href="#services" primary={false}>
            Our capabilities
          </GlowButton>
        </motion.div>
      </motion.div>
      ...
      <motion.div
        ...
        className="absolute bottom-6 left-1/2 z-10 h-10 w-px -translate-x-1/2 overflow-hidden bg-hairline"
      >
        <motion.span
          className="absolute left-0 top-0 h-3 w-px bg-beam"
          animate={{ y: [-12, 44] }}
...
/**
 * The horizon itself — a sphere far larger than the viewport, positioned so
 * only its upper arc crosses the hero. Built from four layers:
 * atmosphere bloom, soft rim glow, the burning rim line, and the dark body.
 */
function Eclipse() {
  // One shared geometry: a huge circle whose apex sits ~78% down the hero.
  const circle =
  ...
  return (
    ...
    >
      {/* 1 — Atmosphere: a wide pool of light breathing above the apex */}
      <div
      ...
      {/* 1b — Hot core: the brightest point of dawn, dead-centre on the apex */}
      <div
      ...
      {/* 2 — The body: the dark sphere itself, eating the light below the rim */}
      <div
        className={circle}
        style={{
          background:
      ...
      {/* 3 — Soft rim glow: a thick, blurred ring hugging the horizon */}
      <div
        className={circle}
        style={{
          background:
      ...
      {/* 4 — The burning rim: a crisp arc, brightest at the apex */}
      <div
        className={circle}
        style={{
          background:
