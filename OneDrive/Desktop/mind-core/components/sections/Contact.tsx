 ...
 *
 * The form is staged as a window in a calm, futuristic operating system:
 * a glass panel with a title bar, a session id, and three short prompts.
 * Filling it should feel like typing into the product Mind Core would build.
 */
export default function Contact() {
  ...
  const onSubmit = (e: FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    // Wire to your endpoint of choice (e.g. /api/contact or a form service).
    setSent(true);
  };

  return (
    <section id="contact" className="relative px-6 pb-44 pt-40 md:pt-56">
      {/* The page's final pool of light rises from below the fold */}
      <div
        aria-hidden
      ...
      <div className="relative mx-auto max-w-6xl">
        <SectionHeader
          ...
          lede="Three prompts. We reply within one working day with a point of view, not a sales sequence."
          align="center"
        />
        ...
        <Reveal className="mx-auto mt-16 max-w-2xl">
          <div className="glass edge-light edge-sweep scanlines relative overflow-hidden rounded-3xl">
            {/* Blueprint grid behind the form */}
            ...
            {sent ? (
              <motion.div
                ...
                role="status"
              >
                <p className="display lit text-2xl font-semibold">Session opened.</p>
                <p className="mx-auto mt-3 max-w-line text-mist">
                  Your message is with the studio. Expect a reply within one
                  working day.
                </p>
              </motion.div>
            ) : (
              <form onSubmit={onSubmit} className="relative z-10 space-y-7 p-8 sm:p-10">
                ...
                <Field
                  id="vision"
                  label="03 · What are we building?"
                  placeholder="A few sentences is enough — we'll ask the right questions."
                  textarea
                ...
                >
                  ...
                  />
                  <span className="relative">Open session →</span>
                </button>
              </form>
            )}
          </div>
...
function Field({
  ...
  const shared =
    "w-full rounded-xl border bg-white/[0.03] px-4 py-3.5 text-[15px] text-frost placeholder:text-mist/40 transition-colors duration-300 focus:outline-none";
  ...
  return (
    <div className="relative">
      <label
        htmlFor={id}
        className={`label mb-2.5 block transition-colors duration-300 ${
          focused ? "text-beam" : ""
        }`}
      >
        {label}
